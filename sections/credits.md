...
## Get credits
`GET /credits.json` will return all your credits.
```json
[
  {
...
* If you want to index the credits for a specific contact you can do it by passing the contact id in the `contact` parameter like `?contact=3231`.
* The `exchange_rate` value is only available if the credit currency differs from your account currency.
...
## Deliver credit
`GET /credits/1/deliver.json` will send the credit to the contact email.
This will return `200 OK` if the deliver was a success along the current JSON representation of the credit with the registered activities for the document. If the destination contact has not  an email you'll see `422` 

## Create credit
`POST /credits.json` will create a new credit from the parameters passed.
```json
{
...
* __contact_id:__ identifier of the contact. Alternatively, __you can use the__ ´contact´ __attribute instead ´contact_id´__. For example:
  ...
  ```
  This have two possible consequences, if the contact first_name and last name combination does not match any of your existing contacts, it will be created, otherwise the credit will be created for the matching result. Please keep in mind that __you can pass__ `contact_id` __or__ `contact` __but not both of them at the same time, results might not be what you've expected.__
...
* __items_attributes:__ An array of hashes which contains the description, quantity, unit price and discount rate of each item. If you want to have stock tracking pass the item code as the `reference` attribute. See more about stock tracking [here](https://github.com/recrea/quaderno-api/blob/master/sections/items.md#create-item). You can also add items to an credit by passing the reference of a pre-created item. See more [here](https://github.com/recrea/quaderno-api/blob/master/sections/items.md#adding-an-item-to-a-document-by-reference).

This will return `201 Created`, with the current JSON representation of the credit if the creation was a success along the location of the new credit in the 'url' field .  If the user does not have access to create new credits, you'll see `401 Unauthorized`.
...
You can set the credit state by passing the `state` attribute, but bear these considerations in mind:

* The `paid` state is only reachable by adding a `payment` to the credit and it is also final, so you cannot overwrite it unless you remove the associated payments.
...
Valid files extension are ```pdf txt jpg jpeg png xml xls doc rtf html```, any other format will make unable the credit creation and will return a ```422```.

## Update credit
`PUT /credits/1.json` will update the credit from the parameters passed.
```json
{
...
This will return `200 OK` if the update was a success along with the current JSON representation of the credit. If the user does not have access to update the credit, you'll see `401 Unauthorized`.

## Delete credit
`DELETE /credits/1.json` will delete the credit specified and return `204 No Content` if that was successful. If the user does not have access to delete the credit, you'll see `401 Unauthorized`.
