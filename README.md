...
## Making a request
All URLs starts with `https://quaderno.com/my-account/api/v1/`. The path is prefixed with the account name.

To make a request for all the contacts in your account for instance, you have to append the contacts index path to the base url to form something like https://quaderno.com/my-account/api/v1/contacts.json. In curl, it would look like:

```shell
curl -u user:pass -X GET 'https://quaderno.com/my-account/api/v1/contacts.json'
```
...
curl -u user:pass 
	 ...
	 'https://quaderno.com/my-account/api/v1/contacts.json'
```
...
```shell
curl -u token-id:foo 'https://quaderno.com/my-account/api/v1/contacts'
```     
...
```shell
curl -u token_id:foo 'https://quaderno.com/my-account/api/v1/ping.json'
```
## Errors
